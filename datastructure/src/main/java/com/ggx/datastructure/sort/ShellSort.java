...
/**
 * 希尔排序
 * 希尔排序的一个重要性质是:一个h次排序的文件(然后将是h-1次排序的)保持它的h的排序性
 * 希尔排序的运行时间依赖于增量序列的选择，而证明很复杂【有兴趣可查看其他资料】。
 * 使用希尔增量时希尔排序最坏时间复杂度是：O(n^2)。最坏情形是列表长度N是2的幂，偶数位置上有N/2个同为最大的数，奇数位置上有N/2个同为最小的数。
 * 使用Hibbard增量(增量为2^k-1)的希尔排序最坏时间复杂度是：O(n3/2)；最优时间复杂度是O(n5/4)。
